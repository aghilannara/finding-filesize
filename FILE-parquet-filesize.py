...
with open('list_of_tables.txt','r') as f:
    ...
    for i in range(0,len(lines)):
        ...
        cmd = 'hdfs dfs -du -s /user/trace/source_files/%s/%s_%s/CURRENT/'%(db,schema,table)
        cmd_his = 'hdfs dfs -du -s /user/trace/source_files/%s/%s_%s/ingest_date*'%(db,schema,table)
        #pdb.set_trace()
